...
**Last updated:** July 13, 2026 (by Bernardo — GitHub migration done)
...
- Decision Log active (D-001 through D-019; D-001 replaced by D-006). ⚠️ Rows D-014 and D-015 missing from the migrated log — recover from Drive version history (see migration note in `decision-log.md`)
...
- Decision Log, Project Brain and Master Plan migrated to GitHub (`F4LA/testimonial-system`) as the single source of truth; Drive docs frozen; pure Markdown, no Pages (D-018, D-019)

## In progress (Phase 2 — first tasks, all parallel from day 1)

- Bernardo: T-E.1 + T-E.2 (process map v0 + dashboard data requirements) · T-0.1 (transcripts + Looms spike)
- Joey: T-B.2 (social templates — critical path, next) · T-A.1 + T-A.2 (client + coach questionnaires)
...
- Joey: T-B.2 social templates → T-B.3 case study → T-A.5 instructions email → T-D.1 / T-D.2 podcast
- Miguel: T-B.4 agent spec — blocked until Joey's social templates (T-B.2) and Bernardo's standard client folder (T-A.3) are ready → T-B.5 build → T-B.6 test

## Blocked

Nothing rock-wide. Miguel's T-B.4 is waiting on inputs from Joey (T-B.2) and Bernardo (T-A.3) — not a rock blocker, just sequencing.
