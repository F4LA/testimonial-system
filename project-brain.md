...
**Last updated:** August 8, 2026 (dashboard Phases 1-3 built + live, D-084; task/alert model redesigned, D-090; coach-form engine bug fixed, D-085; reel moves in-house, Pipix/Sofi out, D-086; Collecting→Producing gate defined, D-087; two-source-of-truth guard + dashboard↔spreadsheet bridge opened, D-088/D-089)
...
- Decision Log active (D-001 through D-090; D-001→D-006, D-018→D-021, D-004, D-049 superseded; D-071, D-072 partially superseded by D-086)
...
- Bernardo: Testimonial Dashboard (task 26) — Phases 1-3 built and live (D-084); read/write/identity/fold all validated. Task engine now being rebuilt to the v2 task model (D-090) before Phase 4. Two open build items alongside the rebuild: the dashboard↔spreadsheet bridge (D-089) and the two-source-of-truth guard (D-088). Phases 4 (calendar+buffer) and 5 (recognitions) not started.
...
- Dashboard (task 26): Phases 1-3 built and live (D-084); build in progress in F4LA/testimonial-dashboard on two open pieces before Phase 4 — the task-engine rebuild to the v2 model (D-090) and the dashboard↔spreadsheet bridge (D-089) — then Phases 4-5. Pre-close gate alongside the backlog (D-032/D-075).
...
- Dashboard↔spreadsheet bridge (D-089) — OPEN: moving a card to Invited in the dashboard should fire the collection fan-out directly, so Gaby never touches the Signal Sheet. Build now if the day allows; plan-B fallback is Gaby keeps ticking the Signal checkbox, connect after. Not an Aug 10 blocker either way.
...
- Pre-launch data wipe (Aug 10, Bernardo, manual): all current dashboard/engine data is test data — wipe by hand in four places before launch: the Event Log (keep header), the 7 test-client Drive folders, the coach-form responses sheet TEST rows, and the Signal sheet. No tracking file needed.
